...
public class MessageGmailBackedPlatformEmailService implements MessagePlatformEmailService {
	...
	public MessageGmailBackedPlatformEmailService(BillingMessageRepository messageDataRepository,
	...
	public String sendToUserEmail(BillingMessageDataForProcessing emailDetail) {
		...
		if (configuration != null) {
			...
			try {
				...
				StringBuilder messageBuilder = new StringBuilder().append(emailDetail.getHeader())
						.append(emailDetail.getBody()).append(emailDetail.getFooter());
	...
	public String sendToUserMobile(String message, Long id, String messageTo, String messageBody) {
		...
		try {
			...
			if (object.has("senderName"))
				senderName = object.getString("senderName");
			if (object.has("senderMessage"))
				senderMessage = object.getString("senderMessage");
			...
			if (null != method && method.equalsIgnoreCase("POST")) {
				 ...
				 * System.out.println(result); if(result.contains(messageTo)) output =
				 * "SUCCESS"; }
				 */
			}
			...
			System.out
					.println("UnsupportedEncodingException : " + e.getMessage() + " . encoding pattern not supported.");
			return e.getMessage();
		} catch (MalformedURLException e) {
			System.out.println("MalformedURLException : " + e.getMessage() + " . URL is not located.");
			return e.getMessage();
		} catch (IOException e) {
	...
	/*
	 * @Override public String sendToMobile(String message, Long id, String
	 * messageTo, String messageBody){ try {
	 * 
	 * URL url = new
	 * URL("https://smsapi.engineeringtgr.com/send/?Mobile="+987764334+
	 * "&Password='Welcome@123'&Message="+messageBody+"test&To="+messageTo+
	 * "&Key=suredUEnM7ervH5L0ZtPTfi"); String urlString =
	 * "http://31.132.7.244:8001/api?username=arewah&password=z19Tk02F&ani=ARWTRL&dnis="
	 * +messageTo+"&message="+messageBody+
	 * "&command=submit&serviceType=&longMessageMode=cut[1]";
	 * System.out.println(urlString); URL url = new URL(urlString); URLConnection
	 * urlcon = url.openConnection(); InputStream stream = urlcon.getInputStream();
	 * int i; String response=""; while ((i = stream.read()) != -1) {
	 * response+=(char)i; } if(response.contains("success")){
	 * System.out.println("Successfully send SMS"); //your code when message send
	 * success }else{ System.out.println(response); //your code when message not
	 * send } } catch (IOException e) { System.out.println(e.getMessage()); } return
	 * null;
	 * 
	 * }
	 */
	...
	public String createEmail(String pdfFileName, String emailId) {
		...
		if (configuration != null) {
			...
			String fileName = "ReportEmail_" + DateUtils.getLocalDateOfTenant().toString().replace("-", "") + "_"
					+ dateTime + ".pdf";

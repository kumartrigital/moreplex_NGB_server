...
import org.mifosplatform.portfolio.plan.exceptions.RechargeFailedException;
		...
		try{
			...
			if(result != null && result.getClientId() !=null){
				...
				if(clientId.equalsIgnoreCase(resourceId)){
					...
					/*
					 * withChanges.put("Result", "FAILURE"); withChanges.put("Description",
					 * "Transaction Failed"); withChanges.put("TransactionId", txnId);
					 */
				...
				/*
				 * withChanges.put("Result", "FAILURE"); withChanges.put("Description",
				 * "Payment Failure"); withChanges.put("TransactionId", txnId);
				 */
				throw new RechargeFailedException();

...
public class ItemDetailsApiResource {
	...
	private final Set<String> RESPONSE_DATA_SERIAL_NUMBER_PARAMETERS = new HashSet<String>(
			Arrays.asList("serialNumber"));
	private final Set<String> RESPONSE_DATA_GRN_IDS_PARAMETERS = new HashSet<String>(Arrays.asList("id"));
	private final Set<String> RESPONSE_ITEM_DETAILS_PARAMETERS = new HashSet<String>(Arrays.asList("id", "purchaseDate",
			"supplierId", "itemMasterId", "orderdQuantity", "receivedQuantity", "id", "itemMasterId", "serialNumber",
			"grnId", "provisioningSerialNumber", "quality", "status", "warranty", "remarks"));
	private final Set<String> RESPONSE_ITEM_MASTER_DETAILS_PARAMETERS = new HashSet<String>(
			Arrays.asList("id", "itemCode", "itemDescription", "chargeCode", "unitPrice"));
	...
	@Autowired
	public ItemDetailsApiResource(final PlatformSecurityContext context,
			final DefaultToApiJsonSerializer<ItemDetailsData> toApiJsonSerializerForItem,
			ApiRequestParameterHelper apiRequestParameterHelper,
			PortfolioCommandSourceWritePlatformService commandsSourceWritePlatformService,
			final GrnReadPlatformService inventoryGrnReadPlatformService,
	...
	public String addItemDetails(final String jsonRequestBody) {
		...
		final CommandWrapper commandRequest = new CommandWrapperBuilder().createInventoryItem(null)
	...
	public String retriveItemDetails(@Context final UriInfo uriInfo, @QueryParam("sqlSearch") final String sqlSearch,
		...
		final Page<ItemDetailsData> clientDatafinal = this.itemDetailsReadPlatformService
				.retriveAllItemDetails(searchItemDetails, officeName, itemCode);
	...
	public String updateEventAction(@PathParam("id") final Long id, final String apiRequestBodyAsJson) {
		final CommandWrapper commandRequest = new CommandWrapperBuilder().updateInventoryItem(id)
	...
	public String deleteItemDetail(@PathParam("id") final Long id, final String apiRequestBodyAsJson) {
		final CommandWrapper commandRequest = new CommandWrapperBuilder().deleteInventoryItem(id)
				.withJson(apiRequestBodyAsJson).build();
		final CommandProcessingResult result = this.commandsSourceWritePlatformService.logCommandSource(commandRequest);
		return this.toApiJsonSerializerForItem.serialize(result);
	}
	...
	@Consumes({ MediaType.APPLICATION_JSON })
	@Produces({ MediaType.APPLICATION_JSON })
	public String retriveItemSerialNumbers(@PathParam("itemmasterId") final Long itemmasterId,
			@PathParam("officeId") final Long officeId, @QueryParam("query") final String query,
			@QueryParam("pairedItemId") final Long pairedItemId, @Context final UriInfo uriInfo) {

		context.authenticatedUser().validateHasReadPermission(resourceNameForPermissionsAllocation);
		List<String> itemSerialNumbers = this.itemDetailsReadPlatformService
				.retriveSerialNumbersOnKeyStroke(itemmasterId, query, officeId, pairedItemId);
		...
		final ApiRequestJsonSerializationSettings settings = apiRequestParameterHelper
				.process(uriInfo.getQueryParameters());
		return this.toApiJsonSerializerForAllocationHardware.serialize(settings, InventoryItemSerialNumberData,
				RESPONSE_DATA_SERIAL_NUMBER_PARAMETERS);
	}
	...
	public String retriveGrnIds(@Context final UriInfo uriInfo) {
		...
		Collection<MCodeData> qualityDatas = this.mCodeReadPlatformService
				.getCodeValue(CodeNameConstants.CODE_ITEM_QUALITY);
		Collection<MCodeData> statusDatas = this.mCodeReadPlatformService
				.getCodeValue(CodeNameConstants.CODE_ITEM_STATUS);
		Collection<ModelProvisionMappingData> modelProvisionMappingData = this.modelProvisionMappingReadPlatformService
				.retrieveAllForDropDown();
		ItemDetailsData itemDetailsData = new ItemDetailsData(inventoryGrnData, qualityDatas, statusDatas, null, null,
				modelProvisionMappingData);
		final ApiRequestJsonSerializationSettings settings = apiRequestParameterHelper
				.process(uriInfo.getQueryParameters());
	...
	public String retriveSwapReason(@Context final UriInfo uriInfo) {
		...
		context.authenticatedUser().validateHasReadPermission(resourceNameForGrnPermissions);
		Collection<MCodeData> swapDevicesReasons = this.mCodeReadPlatformService
				.getCodeValue(CodeNameConstants.SWAP_DEVICE_REASONS);
	...
	public String deAllocateHardware(@PathParam("allocationId") final Long id, final String apiRequestBodyAsJson) {
		...
		final CommandWrapper commandRequest = new CommandWrapperBuilder().deAllocate(id).withJson(apiRequestBodyAsJson)
				.build();
		final CommandProcessingResult result = this.commandsSourceWritePlatformService.logCommandSource(commandRequest);
		return this.toApiJsonSerializerForItem.serialize(result);
	}
	...
	public String retriveItemSerialNumbersBasedOnSerialAndProvisionalSerialNum(@QueryParam("query") final String query,
		...
		return this.toApiJsonSerializerForItem.serialize(settings, itemSerialNumbers,
				RESPONSE_DATA_SERIAL_NUMBER_PARAMETERS);
	}

	@GET
	@Path("searchbyserialnumber")
	@Consumes({ MediaType.APPLICATION_JSON })
	@Produces({ MediaType.APPLICATION_JSON })
	public String retriveItemSerialNumbersBasedOnSerialAndProvisionalsearchbyserialnumber(
			@QueryParam("query") final String query, @Context final UriInfo uriInfo) {

		ItemDetailsData itemSerialNumbers = null;
		ApiRequestJsonSerializationSettings settings = null;
		if (query.length() != 18) {
			throw new SerialNumberNotFoundException(query, null);
		}
		itemSerialNumbers = this.itemDetailsReadPlatformService.retriveSerialNumber(query);
		settings = apiRequestParameterHelper.process(uriInfo.getQueryParameters());
		if (itemSerialNumbers == null) {
			throw new SerialNumberNotFoundException(query, null);
		}
		/*
		 * if(itemSerialNumbers.getStatus().equalsIgnoreCase("In use")) { throw new
		 * SerialNumberNotFoundException("Already Used",query); }
		 */
	...
	@Consumes({ MediaType.APPLICATION_JSON })
	@Produces({ MediaType.APPLICATION_JSON })
	public String retriveItemDetailsBySerialNum(@QueryParam("query") final String query,
			@QueryParam("clientId") final Long clientId, @QueryParam("officeId") final Long officeId,
			@Context final UriInfo uriInfo) {

		context.authenticatedUser().validateHasReadPermission(resourceNameForPermissionsAllocation);
		final ItemData itemMasterData = this.itemDetailsReadPlatformService.retriveItemDetailsDataBySerialNum(query,
				clientId, officeId);
		// final List<FeeMasterData> feeMasterData =
		// this.serviceTransferReadPlatformService.retrieveSingleFeeDetails(clientId,"Deposit");
		...
		return this.toApiJsonSerializerForItemData.serialize(settings, itemMasterData,
				RESPONSE_ITEM_MASTER_DETAILS_PARAMETERS);
	}
	...
	@Consumes({ MediaType.APPLICATION_JSON })
	@Produces({ MediaType.APPLICATION_JSON })
	public String retriveAvailablePairingItems(@PathParam("serialNo") final String serialNo,
			@QueryParam("query") final String query, @Context final UriInfo uriInfo) {

		context.authenticatedUser().validateHasReadPermission(resourceNameForPermissionsAllocation);
		List<String> itemSerialNumbers = this.itemDetailsReadPlatformService
				.retriveSerialNumbersOnKeyStrokeForpairedItems(serialNo, query);
		ItemSerialNumberData InventoryItemSerialNumberData = new ItemSerialNumberData(itemSerialNumbers);
		final ApiRequestJsonSerializationSettings settings = apiRequestParameterHelper
				.process(uriInfo.getQueryParameters());
		return this.toApiJsonSerializerForAllocationHardware.serialize(settings, InventoryItemSerialNumberData,
	...
	public String swapDevice(final String apiRequestBodyAsJson) {
		...
		final CommandWrapper commandRequest = new CommandWrapperBuilder().swapDevice().withJson(apiRequestBodyAsJson)
				.build();
		final CommandProcessingResult result = this.commandsSourceWritePlatformService.logCommandSource(commandRequest);
		return this.toApiJsonSerializerForItem.serialize(result);
	}
	...
	@GET
	@Path("itemsale/{itemSaleId}")
	@Consumes({ MediaType.APPLICATION_JSON })
	@Produces({ MediaType.APPLICATION_JSON })
	public String getitemsaleitems(@PathParam("itemSaleId") Long itemSaleId, @Context final UriInfo uriInfo) {

		final List<ItemDetailsData> ItemDetailsData = this.itemDetailsReadPlatformService
				.retriveByItemSaleId(itemSaleId);
		System.out.println("ItemDetailsApiResource.getitemsaleitems()" + ItemDetailsData);
		
		ApiRequestJsonSerializationSettings settings = apiRequestParameterHelper.process(uriInfo.getQueryParameters());

		return this.toApiJsonSerializerForItem.serialize(settings, ItemDetailsData,
				RESPONSE_DATA_SERIAL_NUMBER_PARAMETERS);
	}

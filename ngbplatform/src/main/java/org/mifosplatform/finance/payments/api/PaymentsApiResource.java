...
public class PaymentsApiResource {
	 ...
	 */
	private static final Set<String> RESPONSE_DATA_PARAMETERS = new HashSet<String>(Arrays.asList("id", "clientId",
			"paymentDate", "paymentCode", "amountPaid", "statmentId", "externalId", "Remarks"));
	...
	@Autowired
	public PaymentsApiResource(final PlatformSecurityContext context,
	...
	public String createPayment(@PathParam("clientId") final Long clientId, final String apiRequestBodyAsJson) {
		final CommandWrapper commandRequest = new CommandWrapperBuilder().createPayment(clientId)
	...
	@Consumes({ MediaType.APPLICATION_JSON })
	@Produces({ MediaType.APPLICATION_JSON })
	public String retrieveAllDetailsForPayments(@PathParam("clientId") final Long clientId,
			@Context final UriInfo uriInfo) {
		context.authenticatedUser().validateHasReadPermission(RESOURCENAMEFORPERMISSIONS);
	...
	public String cancelPayment(@PathParam("paymentId") final Long paymentId, final String apiRequestBodyAsJson) {
		...
		final CommandWrapper commandRequest = new CommandWrapperBuilder().cancelPayment(paymentId)
				.withJson(apiRequestBodyAsJson).build();
		final CommandProcessingResult result = this.writePlatformService.logCommandSource(commandRequest);
		return this.toApiJsonSerializer.serialize(result);
	}
	...
	public String dalpayCheckout(final String apiRequestBodyAsJson) {
		...
		try {
			...
			return "<!-- success--> <span>Order Accepted Successfully</span>" + "<br>" + "<a href='" + returnUrl + "'>"
	...
	public String paypalEnquireyPayment(@PathParam("clientId") final Long clientId, final String apiRequestBodyAsJson) {
		...
		final CommandWrapper commandRequest = new CommandWrapperBuilder().paypalEnquireyPayment(clientId)
				.withJson(apiRequestBodyAsJson).build();
		final CommandProcessingResult result = this.writePlatformService.logCommandSource(commandRequest);
		return this.toApiJsonSerializer.serialize(result);
	}
	...
	@GET
	@Path("paymenttypes/{officeId}")
	@Consumes({ MediaType.APPLICATION_JSON })
	@Produces({ MediaType.APPLICATION_JSON })
	public String paymenttypes(@PathParam("officeId") final Long officeId, @Context final UriInfo uriInfo,
			final String apiRequestBodyAsJson) throws JSONException {

		final BigDecimal onlinePayment = readPlatformService.getPaymentDetails(officeId);
		final BigDecimal voucherPayment = readPlatformService.getVoucherDetails(officeId);

		System.out.println("PaymentsApiResource.paymenttypes() onlinePayment:" +onlinePayment +"voucherPayment:"+voucherPayment);
		JSONObject result = new JSONObject();
		result.put("onlinePayment", onlinePayment);
		result.put("voucherPayment", voucherPayment);
